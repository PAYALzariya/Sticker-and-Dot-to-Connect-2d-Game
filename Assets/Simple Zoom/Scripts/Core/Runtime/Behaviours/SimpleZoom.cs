...
namespace DanielLochner.Assets.SimpleZoom
{
    ...
    public class SimpleZoom : MonoBehaviour, IPointerClickHandler
    {
        ...
        [SerializeField] public Vector2 customPosition = new Vector2(0.5f, 0.5f);
        ...
        private bool IsValidConfig
        {
            get
            {
                ...
                {
                    Debug.Log("<b>[SimpleZoom]</b> The world camera must be assigned if the render mode has been set to \"Screen Space - Camera\".", gameObject);
                    valid = false;
                }
                if (zoomView != null)
                {
